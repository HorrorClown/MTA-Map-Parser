...
function CMapParser:constructor(sResourceName, client)
    ...
    --[[
        ToDo: If a map was already converted/parsed, check the parserVersion. If the parserVersion from map is lower than this, the map may require an upgrade.
        ToDo: That means, there are different changes and the map does not need to get completely parsed again. Special functions for specific versions still needed. Too lazy to do that now O:
    ]]
        ...
        for k, v in pairs(self.meta) do
            ...
            elseif k == "settings" then
                ...
                for _, sv in ipairs(v) do
                    ...
                    --I could make that more efficient, but i am too lazy now O:
                       if Core:getManager("CMCManager").validSettings[self.mapType][sv.name] then
                           setting:setAttribute("value", Core:getManager("CMCManager").validSettings[self.mapType][sv.name])
                       else
...
function CMapParser:generatePewScript()
    self.pewScript = ([[--
-- This file was automatically generated by PewX' Map Parser Resource: https://github.com/HorrorClown/MTA-Map-Parser
-- HorrorClown (PewX)
-- Using: IntelliJ IDEA 14 Ultimate
-- Date: 10.05.2015 - Time: 06:27
-- pewx.de // iGaming-mta.de // iRace-mta.de // iSurvival.de // mtasa.de
